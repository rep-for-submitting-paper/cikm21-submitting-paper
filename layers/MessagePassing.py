...
class SageLayer(nn.Module):
    def __init__(self, in_dim, out_dim, drop_rate=0.4, cuda=False, cuda_device="cuda:0"):
        super(SageLayer, self).__init__()
        self.drop_rt = drop_rate
        self.cuda = cuda
        self.device = cuda_device
        self.weight = nn.Parameter(torch.FloatTensor(out_dim, in_dim * 2)).to(self.device)
