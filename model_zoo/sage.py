...
class GraphSage(nn.Module):
    def __init__(self, adj_lists, feats, num_classes, embed_dim, num_sample, num_layers, is_cuda, as_view=False, cuda_device="cuda:0"):
        ...
        self.raw_features = feats  # 能够改成sparse
        ...
        self.sage_layer1 = SageLayer(in_dim=self.feat_dim, out_dim=self.embed_dim, cuda=self.is_cuda, cuda_device=self.device)
        self.sampler.add_sample_layer(num_sample=self.num_sample)

        self.sage_layer2 = SageLayer(in_dim=self.embed_dim, out_dim=self.embed_dim, cuda=self.is_cuda, cuda_device=self.device)
        self.sampler.add_sample_layer(num_sample=self.num_sample)
    ...
    def get_embedding(self, nodes):
        ...
        pre_hidden_embs = torch.Tensor(self.raw_features[unique_nodes_list].todense())
        if self.is_cuda:
            pre_hidden_embs = pre_hidden_embs.to(self.device)
        # print(pre_hidden_embs)
        # print(pre_hidden_embs.sum())
...
class GraphSageHandler(BaseHander):
    ...
    def __init__(self, num_class, data, args):
        ...
        self.ddc_target_data_loader = self.inputdata.get_ddc_target_data_load(batch_size=args.batch_size, shuffle=True)
    ...
    def build_model(self):
        ...
        self.loss_func = nn.CrossEntropyLoss(weight=torch.Tensor([1, 5]).cuda())
